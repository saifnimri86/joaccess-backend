...
def create_app(config_class: type = Config) -> Flask:
    ...
    from cv_blueprint import cv_api  # NEW — computer-vision verification
 
    app.register_blueprint(mobile_api, url_prefix="/api/v1")
    app.register_blueprint(admin_api, url_prefix="/api/admin")
    # cv_api also lives under /api/admin so it inherits the admin URL space
    # (its routes already start with /cv/... or /locations/<id>/analyze).
